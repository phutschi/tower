...
describe("fold", () => {
  ...
  test("blocked needs attention; stale needs attention; blocked is never stale", () => {
    const s = fold(
      run,
      [
        report("1", "blocked", 0, { note: "need db" }),
        report("2", "in_progress", 5, { model: "m" }),
      ],
  ...
  test("reviewing goes stale too; pending and done never do", () => {
    const s = fold(
      run,
      [report("1", "reviewing", 0, { model: "m" }), report("2", "done", 0)],
      { now: new Date(at(30)), staleMinutes: 10 },
    );
    expect(s.tasks[0]?.stale).toBe(true);
    expect(s.tasks[1]?.stale).toBe(false);
    expect(s.tasks[2]?.stale).toBe(false);
  });
